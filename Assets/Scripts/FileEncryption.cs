...
public class FileEncryption : MonoBehaviour
{
    void Start()
    {
        ...
        using (FileStream fsWrite = File.OpenWrite(encryptedPath))
        {
            int byteData = fsRead.ReadByte();
            while (byteData != -1)
            {
                byteData ^= 0xAB;
                fsWrite.WriteByte((byte)byteData);
                byteData = fsRead.ReadByte();
            }
        }

        //3
        string decryptedPath = Path.Combine(Application.persistentDataPath, "decrypted.txt");
        using (FileStream fsRead = File.OpenRead(encryptedPath))
        using (FileStream fsWrite = File.OpenWrite(decryptedPath))
        {
            int byteData = fsRead.ReadByte();
            while (byteData != -1)
            {
        ...
        //4
        Debug.Log("원본: " + File.ReadAllText(path));
        Debug.Log("암호화 완료 (파일 크기: " + new FileInfo(encryptedPath).Length + " bytes)");
        Debug.Log("복호화 완료");
        Debug.Log("복호화 결과: " + File.ReadAllText(decryptedPath));
        Debug.Log("원본과 일치: " + (File.ReadAllText(path) == File.ReadAllText(decryptedPath)));
    }
}
